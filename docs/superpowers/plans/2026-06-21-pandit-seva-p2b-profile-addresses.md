...
**Cross-phase wiring deferred to P2c (flagged by the final whole-branch review):** the two new account settings persist state but are not yet read by the subsystems they describe — (a) `uiStore.phoneShareDefault` is not consumed by `chatStore` when creating a new booking thread (new threads still default `phoneShared: false`); (b) `bookingStore.getDefaultAddress()` is defined + tested but not wired into `startDraft`, so the default address is not auto-pre-selected in the booking flow. Both are intentional scope boundaries (chat = P2a, booking = P1c subsystems, outside P2b's Profile/Addresses scope); wire them in P2c or a P5 integration pass.
